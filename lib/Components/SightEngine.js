...
export default {
   detectImage: async (filePath) => {
      ...
      return data.nudity?.sexual_activity >= MAX_SCORE ||
         data.nudity?.erotica >= MAX_SCORE ||
         data.nudity?.suggestive >= MAX_SCORE ||
         data.nudity?.very_suggestive >= MAX_SCORE ||
         data.gore?.prob >= MAX_SCORE ||
         data.violence?.prob >= MAX_SCORE
   },
