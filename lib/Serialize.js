...
export const Serialize = (sock, store, setting, m) => {
   if (m.key) {
      m.id = m.key.id
      m.chat = findChatId(m.key)

      const sender = findUserJid(sock, m.key)
      const senderLid = findUserLid(sock, m.key)
   ...
   let innerMessage = normalizeMessageContent(m.message)
   ...
   m.msg = innerMessage[messageType]
   m.type = messageType
   ...
   m.pushName = m.verifiedBizName || m.pushName || 'Somebody'

   const contextInfo = typeof m.msg !== 'string' ?
      m.msg?.contextInfo :
      null
