...
import { delay, DONATE_URL, isJidGroup, prepareWAMessageMedia, S_WHATSAPP_NET, WA_DEFAULT_EPHEMERAL } from '@itsliaaa/baileys'
...
import { SECOND, MINUTE, HOUR, DAY, BRAT_GIF_ARGS, FFMPEG_CONCAT_ARGS, MENTION_REGEX, READ_HEADER_LIMIT, URL_REGEX, WEBP_EXIF_HEADER, WHATSAPP_URL_REGEX, IMAGE_TO_WEBP, VIDEO_TO_WEBP, AUDIO_TO_MPEG, AUDIO_TO_OPUS, FONT_MAPS } from './Constants.js'
...
export const persistToFile = async (source) => {
   ...
   const fileName = resolve(process.cwd(), temporaryFolder, createFileName()) + '.tmp'
...
export const resizeImage = async (media, width = 540, height = null, quality = 70, format = 'jpeg') => {
...
export const createSticker = async (media, options = {}) => {
   ...
   const mimetype = options.mimetype || (await fileTypeFromFile(media))?.mime
...
export const bratVideoSticker = async (text = 'Hi') => {
   ...
   const files = await Promise.all(
      texts.map((_, index) =>
         persistToFile(`https://aqul-brat.hf.space/?text=${encodeURIComponent(texts.slice(0, index + 1).join(' '))}`)
      )
   )
...
export const imageToWebP = (media) =>
   ffmpeg(
      media,
      [],
      IMAGE_TO_WEBP,
...
export const videoToWebP = (media) =>
   ffmpeg(
      media,
      [],
      VIDEO_TO_WEBP,
...
export const ExtendSocket = (sock, { updatePresence, delayWithPresence, secureMetaServiceLabel }) => {
   ...
   sock.sendMessage = async (jid, content, options) => {
      if (
         ...
         const presenceType = content.ptt ?
            'recording' :
            'composing'
   ...
   sock.sendText = async (jid, text = '', quoted, content = {}, options = {}) => {
      text =
         typeof text === 'string' ?
            text :
      ...
      if (content.thumbnail) {
         ...
         const matchedText = content.thumbnailUrl ?? DONATE_URL
         text = matchedText + ' ' + text

         linkPreview['matched-text'] = matchedText
         linkPreview.title = content.title ?? botName
         linkPreview.description = content.description
         linkPreview.previewType = content.previewType

         const savedImage = await persistToFile(content.thumbnail ?? botThumbnail)
         linkPreview.jpegThumbnail = await resizeImage(savedImage, 200, null)
   ...
   sock.sendMedia = async (jid, source, caption = '', quoted, content = {}, options = {}) => {
      try {
         ...
         if (content.sticker) {
            ...
            media = { url: source }
            mimetype = content.mimetype || (await fileTypeFromFile(source))?.mime || 'text/plain'
         ...
         console.error(error)
         return sock.sendText(jid, '❌ Failed to get data : ' + error.message, quoted)
      }
   }
